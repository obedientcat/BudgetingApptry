...
// Helpers
// ----------
// Categories are user-defined (advanced mode). We keep a couple of reserved defaults.
var RESERVED_CATEGORIES = ['Others', 'Savings'];
...
var budgetController = (function () {
    ...
    var recalcCategorySpent = function () {
        ...
        data.allItems.exp.forEach(function (e) {
            var cat = normalizeCategory(e.category) || 'Others';
    ...
    var migrateIfNeeded = function () {
        ...
        if (data.categories.limits['Others'] === undefined) data.categories.limits['Others'] = 0;
        if (data.categories.spent['Others'] === undefined) data.categories.spent['Others'] = 0;
        ...
        if (data.allItems && data.allItems.exp) {
            data.allItems.exp.forEach(function (e) {
                e.category = normalizeCategory(e.category) || 'Others';
                if (!e.meta) e.meta = {};
            });
        }
        recalcCategorySpent();
    ...
    return {
        addItem: function (type, des, val, category, meta) {
            ...
            if (type === 'exp') {
                var cat = normalizeCategory(category) || 'Others';
        ...
        deleteCategory: function (category) {
            var cat = normalizeCategory(category);
            if (!cat) return;
            if (cat === 'Others' || cat === 'Savings') return;

            // move any existing expenses in this category to Others
            data.allItems.exp.forEach(function (e) {
        ...
        getCategorySummary: function () {
            ...
            var namesMap = {};
            Object.keys(data.categories.limits || {}).forEach(function (k) { namesMap[k] = true; });
            Object.keys(data.categories.spent || {}).forEach(function (k) { namesMap[k] = true; });
            ...
            // ensure Others always first
            names = names.filter(function (n) { return n !== 'Others'; });
            names.sort(function (a, b) { return a.localeCompare(b); });
            names.unshift('Others');
...
var UIController = (function () {
    ...
    return {
        getInput: function () {
            ...
            var category = categoryEl && !categoryEl.hasAttribute('hidden') ? normalizeCategory(categoryEl.value) : 'Others';
        ...
        renderCategoryLimits: function (summary, limitsMap) {
            ...
            summary.forEach(function (row) {
                ...
                var canDelete = row.category !== 'Others';
                var html = `
